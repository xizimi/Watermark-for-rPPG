...
class SignalProcessing():
    ...
    def __init__(self):
        ...
        self.skin_extractor = SkinExtractionConvexHull('GPU')
        # Patches parameters #
    ...
    def extract_holistic_yuv(self, videoFileName, w_h):
        """
        ...
        with mp_face_mesh.FaceMesh(
                ...
                min_detection_confidence=0.5,
                min_tracking_confidence=0.5) as face_mesh:
            for frame in extract_frames_yield_yuv(videoFileName, w_h):
                # convert the BGR image to RGB.
                image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # 修改代码
                processed_frames_count += 1
                width = image.shape[1]
                height = image.shape[0]
                # [landmarks, info], with info->x_center ,y_center, r, g, b
        ...
        sig = np.array(sig, dtype=np.float32)
        return sig
def extract_frames_yield_yuv(videoFileName, w_h):

    # 直接读取YUV420，转为BGR
    width, height = w_h
    yuv_file = open(videoFileName, 'rb')
    ...
    while True:
        ...
        YUV_image.append(yuv_image)
        bgr_image.append(cv2.cvtColor(yuv_image, cv2.COLOR_YUV2BGR_I420))
    yuv_file.close()
    return bgr_image
